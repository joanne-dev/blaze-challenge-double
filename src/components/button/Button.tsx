...
export const Button = ({ text, type, onClick, className }: ButtonProps) => {
  switch (type) {
    ...
    case 'gameMode': {
      return (
        <ButtonStyled $background={'#000'} color={'#0F1923'} $fontSize={'10px'} onClick={onClick} className={className}>
          {text}
        </ButtonStyled>
      )
    }
...
export const BetButton = ({ text, multiplier, icon, onClick, className, disabled, type }: BetButtonProps) => {
  switch (type) {
    case 'betRed': {
      return (
        <ButtonStyled
